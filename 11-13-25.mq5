...
int g_UseSupertrendOnLastLevel = -1; // Use Supertrend on last level - -1 means use input parameter (0=false, 1=true)
...
int OnInit()
{
   ...
   g_UseSupertrendOnLastLevel = inpUseSupertrendOnLastLevel ? 1 : 0;
   ...
   if(GetActiveUseSupertrendOnLastLevel())
   {
...
void ManagePartialTPExecution()
{
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      bool supertrendManagingAll = (GetActiveNumberOfLevels() == 1 && GetActiveUseSupertrendOnLastLevel());
      ...
      if(!supertrendManagingAll && !HasExecutedTPLevel(ticket, 1) && calc.partialTP1Price > 0)
      {
         ...
         if(tp1Reached)
         {
            ...
            bool isLastTP = (GetActiveNumberOfLevels() == 1);
            ...
            if(GetActiveUseSupertrendOnLastLevel() && isLastTP)
            {
            ...
            else if(ExecutePartialClose(ticket, inpExitPercent1, 1, calc.partialTP1Price, g_PartialLots1, isLastTP))
            {
               ...
               bool nextLevelIsLast = (GetActiveNumberOfLevels() == 2);  // With 2 levels, TP2 is last

               if(GetActiveUseSupertrendOnLastLevel() && nextLevelIsLast)
               {
      ...
      if(!HasExecutedTPLevel(ticket, 2) && calc.partialTP2Price > 0)
      {
         ...
         if(tp2Reached)
         {
            ...
            bool isLastTP = (GetActiveNumberOfLevels() == 2);
            ...
            else if(ExecutePartialClose(ticket, inpExitPercent2, 2, calc.partialTP2Price, g_PartialLots2, isLastTP))
            {
               ...
               // If so, hand over the remaining position to Supertrend management
               bool nextLevelIsLast = (GetActiveNumberOfLevels() == 3);  // With 3 levels, TP3 is last
      ...
      if(GetActiveNumberOfLevels() == 3 && !HasExecutedTPLevel(ticket, 3) && calc.partialTP3Price > 0)
      {
...
void UpdatePanel()
{
   ...
   if(inpShowTP)
   {
      ...
      bool skipTP2Display = (GetActiveNumberOfLevels() == 2 && GetActiveUseSupertrendOnLastLevel());
      bool skipTP3Display = (GetActiveNumberOfLevels() == 3 && GetActiveUseSupertrendOnLastLevel());
      ...
      if(GetActiveNumberOfLevels() >= 2 && !skipTP2Display)
      {
      ...
      if(GetActiveNumberOfLevels() == 3 && !skipTP3Display)
      {
...
bool GetActiveUseSupertrendOnLastLevel()
{
   return (g_UseSupertrendOnLastLevel >= 0) ? (g_UseSupertrendOnLastLevel == 1) : inpUseSupertrendOnLastLevel;
}
...
void CalculatePartialExits(RiskCalculation &calc, double pipValuePerLot, bool isLongTrade)
{
   ...
   bool supertrendManagingLastLevel = GetActiveUseSupertrendOnLastLevel();
...
void UpdateLines()
{
   ...
   if(inpShowTP && !supertrendManagingAll)
   {
      ...
      bool skipTP2 = (GetActiveNumberOfLevels() == 2 && GetActiveUseSupertrendOnLastLevel());

      if(GetActiveNumberOfLevels() >= 2 && !skipTP2)
      {
      ...
      bool skipTP3 = (GetActiveNumberOfLevels() == 3 && GetActiveUseSupertrendOnLastLevel());

      if(GetActiveNumberOfLevels() == 3 && !skipTP3)
      {
...
void LoadSettingsFromFile(bool loadPercentages = true)
{
   ...
   while(!FileIsEnding(fileHandle))
   {
      ...
      if(equalPos > 0 && section == "GlobalVariables")
      {
         ...
         else if(key == "UseSupertrendOnLastLevel")
            g_UseSupertrendOnLastLevel = (int)StringToInteger(value);
...
void SaveSettingsToFile()
{
   ...
   FileWriteString(fileHandle, "UseSupertrendOnLastLevel=" + IntegerToString(g_UseSupertrendOnLastLevel) + "\n");
...
{
   if(!GetActiveUseSupertrendOnLastLevel() || !inpSupertrendTrailingStop)
      return;
...
{
   if(!GetActiveUseSupertrendOnLastLevel() || !inpShowSupertrendLine)
      return;
