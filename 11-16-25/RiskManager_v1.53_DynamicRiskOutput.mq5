...
#property description "3-Level Dynamic Risk Management with Integrated Trade Planning and Dynamic Risk Output"
...
string StateToCsv(const RiskManagerState &state) {
    return StringFormat(
        "%.6f,%.6f,%.6f,%.6f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%d,%d,%d,%d,%s,%llu,%s,%d",
        ...
        state.monthlyGoalAmount,
        state.weeklyGoalPercent,
        state.weeklyGoalAmount,
        state.dailyGoalPercent,
        state.dailyGoalAmount,
        state.currentMonthPnL,
        state.currentWeekPnL,
        state.currentDayPnL,
...
bool CsvToState(const string csvStr, RiskManagerState &state) {
    ...
    if(count != 28) {
        Print("❌ Invalid state file format. Expected 28 fields, got ", count);
        return false;
    }
    ...
    state.monthlyGoalAmount = StringToDouble(parts[12]);
    state.weeklyGoalPercent = StringToDouble(parts[13]);
    state.weeklyGoalAmount = StringToDouble(parts[14]);
    state.dailyGoalPercent = StringToDouble(parts[15]);
    state.dailyGoalAmount = StringToDouble(parts[16]);
    state.currentMonthPnL = StringToDouble(parts[17]);
    state.currentWeekPnL = StringToDouble(parts[18]);
    state.currentDayPnL = StringToDouble(parts[19]);
    state.monthStart = (datetime)StringToInteger(parts[20]);
    state.weekStart = (datetime)StringToInteger(parts[21]);
    state.dayStart = (datetime)StringToInteger(parts[22]);
    state.consecutiveLosses = (int)StringToInteger(parts[23]);
    state.lastTradeType = parts[24];
    state.lastProcessedTicket = StringToInteger(parts[25]);
    state.accountNumber = parts[26];
    state.lastUpdateTime = (datetime)StringToInteger(parts[27]);
...
void HandleLoss(RiskManagerState &state, double lossAmount, datetime closeTime) {
    ...
    if(state.currentRiskPercent < previousRisk) {
        ...
        // Update dynamic risk output file with new risk level
        UpdateRiskFile(state.currentRiskPercent);
    }
}
...
void CreateDisplay() {
    ...
    // Calculate panel height based on whether drawdown is shown
    int panelHeight;
    if(inpShowDrawdown) {
        panelHeight = 263; // Height with drawdown line
    } else {
        panelHeight = 245; // Height without drawdown line
    }
    ...
    // Calculate number of labels needed based on drawdown setting
    int labelCount = inpShowDrawdown ? 10 : 9;

    // Create text labels
    if(inpShowDrawdown) {
        // With drawdown: 10 labels (Title, Level, Drawdown, Targets, Detail1-3, TradePlanning1-3)
        string lineNames[10] = {"_Title", "_DD", "_Risk", "_Targets", "_Detail1", "_Detail2", "_Detail3", "_TradePlanning1", "_TradePlanning2", "_TradePlanning3"};
        for(int i = 0; i < 10; i++) {
        ...
        // Without drawdown: 9 labels (Title, Level, Targets, Detail1-3, TradePlanning1-3)
        string lineNames[9] = {"_Title", "_DD", "_Targets", "_Detail1", "_Detail2", "_Detail3", "_TradePlanning1", "_TradePlanning2", "_TradePlanning3"};
        for(int i = 0; i < 9; i++) {
            string labelName = g_labelName + lineNames[i];
            if(ObjectCreate(0, labelName, OBJ_LABEL, 0, 0, 0)) {
                int textX = panelX + textPadding;
...
void UpdateDisplay() {
    ...
    if(inpShowDrawdown) {
        ...
        // When drawdown is disabled, use the Risk label as the divider
        ObjectSetString(0, g_labelName + "_Targets", OBJPROP_TEXT, "--------------");
    }

    int offsetIndex = inpShowDrawdown ? 0 : -1; // Adjust label index when drawdown is disabled
...
void DeleteDisplay() {
    ...
    // Delete labels based on whether drawdown was enabled
    if(inpShowDrawdown) {
        // Delete all 10 labels used when drawdown is enabled
        string lineNames[10] = {"_Title", "_DD", "_Risk", "_Targets", "_Detail1", "_Detail2", "_Detail3", "_TradePlanning1", "_TradePlanning2", "_TradePlanning3"};
        for(int i = 0; i < 10; i++) {
            ObjectDelete(0, g_labelName + lineNames[i]);
        }
    } else {
        // Delete only 9 labels used when drawdown is disabled
        string lineNames[9] = {"_Title", "_DD", "_Targets", "_Detail1", "_Detail2", "_Detail3", "_TradePlanning1", "_TradePlanning2", "_TradePlanning3"};
        for(int i = 0; i < 9; i++) {
            ObjectDelete(0, g_labelName + lineNames[i]);
        }
...
int OnInit() {
    ...
    Print("🚀 Risk Manager Indicator (Integrated Trade Planning & Dynamic Risk Output) starting on ", _Symbol);
    ...
    g_initialized = true;
    Print("✅ Risk Manager Indicator initialized with Dynamic Risk Output");
